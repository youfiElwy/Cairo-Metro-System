...
module.exports = function (app) {
	...
	app.post('/api/v1/users/forgot_password', async function (req, res) {
		...
		if (isEmpty(userExists)) {
			return res.status(401).send([401, 'Incorrect user information. You are not the user!']);
		}
		// If verification is successful, proceed with password reset
		try {
			const token = crypto.randomBytes(3).toString('hex').toString(); // Generate a secure token
				...
				html: `
				  <html>
					 ...
					 <body>
						<div class="container">
						  <h1>Hello, citizen!</h1>
						  <p>Here is your unique validation token.</p>
						  <p>Proceed to enter it in the website!</p>
						  <p><a class="button" href="${token}">${token}</a></p>
						</div>
					 </body>
				  </html>
				`,
			...
			return res.status(200).send([200, resetLink]);
		} catch (err) {
