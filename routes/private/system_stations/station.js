...
module.exports = function (app) {
	...
	app.post('/station', async (req, res) => {
		try {
			...
			const found = (await db('station').where('description', description).select()).length;

			if (found === 1) {
				res.status(402).send('This station is already exists ');
			}
	...
	app.put('/station/:id', async (req, res) => {
		try {
			...
			const found = await db('station').where({ description: old_description });

			if (found.length === 0) {
				res.status(402).send('This station does NOT exist');
			}
			const found2 = await db('station').where({
				description: new_description,
			});
			if (found2.length !== 0) {
				res.status(403).send('This Name is taken choose other name');
			}
			await db('station')
				.where('description', old_description)
				.update({ description: new_description, admin_id: admin_id });

			res.status(200).json('statoin updated');
		} catch (error) {
			console.error(error.message);
		}
	});
	...
	app.delete('/station', async (req, res) => {
		try {
			...
			const isValid = (await db('station').where('description', target_station).select()).length;
			if (!isValid) {
				res.status(402).send('no valid station');
			}
			...
			await db('station').where({ description: target_station }).del();
			await rerun_pricing_algo();
			console.log('hee');
			res.status(200).json('deleted'); // to return the res to user
		} catch (error) {
			console.error(error.message);
		}
	});
	...
	async function updating_the_matrix() {
		...
		const adjMatrix2 = Array.from({ length: size }, () => Array(size).fill(false));
